class PurchaseRecordAddress
  ...
  with_options presence: true do
    validates :postal_code,
              format: { with: /\A[0-9]{3}-[0-9]{4}\z/, message: 'is invalid. Input half-width number include hyphen(-)' }
    validates :prefecture_id, numericality: { other_than: 1, message: "can't be blank" }
    validates :city
    validates :address
    validates :phone_number,
              numericality: { only_integer: true, message: 'is invalid. Input half-width number except hyphen(-)' }, length: { minimum: 10, maximum: 11, message: 'does not fit in setting count.' }
    validates :item_id
    validates :user_id
    validates :token
  end
